import {
  ...
  const signalsConfig = [
    // Note: Many signal conditions have been "relaxed" to trigger more frequently.
    // =================================================================
    // Category: Volatility & Fear Signals
    // =================================================================
    { name: 'Extreme Fear (Panic)', type: 'critical', region: 'Global', desc: 'The VIX is above 30, signaling panic or capitulation in the market.', action: 'High-risk environment. Consider closing risky positions or buying protection (puts).', check: d => d.volatility?.VIX?.price > 30 },
    { name: 'High Fear', type: 'critical', region: 'Global', desc: 'The VIX is above 22, signaling significant market fear.', action: 'Reduce long exposure, consider short positions, or buy put options.', check: d => d.volatility?.VIX?.price > 22 && d.volatility?.VIX?.price <= 30 },
    { name: 'Elevated Fear', type: 'warning', region: 'Global', desc: 'The VIX is above 18, indicating higher-than-average market stress.', action: 'Tighten stop-losses on existing long positions.', check: d => d.volatility?.VIX?.price > 18 && d.volatility?.VIX?.price <= 22 },
    { name: 'Market Complacency', type: 'warning', region: 'Global', desc: 'The VIX is below 14, indicating low fear and high investor confidence.', action: 'Consider taking partial profits on long positions.', check: d => d.volatility?.VIX?.price < 14 },
    { name: 'Indian Volatility Spike', type: 'critical', region: 'India', desc: d => `India VIX has surged by ${d.volatility?.['India VIX']?.changePercent?.toFixed(1)}%, indicating a sharp increase in expected volatility.`, action: 'Traders may become cautious on Indian equities, reduce leverage.', check: d => d.volatility?.['India VIX']?.changePercent > 10 },
    { name: 'Indian Market Calm', type: 'info', region: 'India', desc: 'The India VIX is below 12, suggesting a stable and low-volatility environment.', action: 'Favorable environment for option sellers and trend-following strategies.', check: d => d.volatility?.['India VIX']?.price < 12 },
    { name: 'Volatility Crush', type: 'info', region: 'Global', desc: 'The VIX has fallen more than 10% today, often occurring after a major risk event has passed.', action: 'Often bullish for equities. Consider selling put options to collect premium.', check: d => d.volatility?.VIX?.changePercent < -10 },
    ...
    { name: 'Volatility Threshold Breached', type: 'info', region: 'Global', desc: 'The VIX has crossed above the key 20 level, signaling a transition to a high-volatility regime.', action: 'Expect wider trading ranges. Volatility-based strategies may become more attractive.', check: d => d.volatility?.VIX?.price > 20 },
    ...
    { name: 'Broad Market Rally', type: 'opportunity', region: 'Global', desc: 'Major US indices are all up strongly.', action: 'A strong bullish signal. Consider buying broad market ETFs.', check: d => d.benchmarks?.['S&P 500']?.changePercent > 0.8 && d.benchmarks?.Nasdaq?.changePercent > 1.0 && d.benchmarks?.['Russell 2000']?.changePercent > 1.0 },
    { name: 'Market Capitulation', type: 'critical', region: 'Global', desc: 'The S&P 500 is down more than 2.0%, a sign of panic selling.', action: 'Contrarian buyers look for these moments to start accumulating positions for a potential bounce.', check: d => d.benchmarks?.['S&P 500']?.changePercent < -2.0 },
    { name: 'Market Euphoria', type: 'warning', region: 'Global', desc: 'The S&P 500 is up more than 2.0%, a sign of euphoric buying.', action: 'A signal to be cautious. Consider taking profits or tightening stop-losses.', check: d => d.benchmarks?.['S&P 500']?.changePercent > 2.0 },
    ...
    { name: 'Emerging Market Strength', type: 'opportunity', region: 'Intermarket', desc: 'Emerging Markets are outperforming the S&P 500.', action: 'Bullish for non-US assets. Traders might look for opportunities in EEM.', check: d => d.specialized?.['Emerging Markets']?.changePercent > d.benchmarks?.['S&P 500']?.changePercent + 0.5 },
    { name: 'Weak Market Breadth', type: 'warning', region: 'Global', desc: 'The Nasdaq is up but the broader Russell 2000 is down, an unhealthy sign.', action: 'A sign of a fragile rally. Be cautious, as the rally may not be sustainable.', check: d => d.benchmarks?.Nasdaq?.changePercent > 0.5 && d.benchmarks?.['Russell 2000']?.changePercent < 0 },
    ...
    { name: 'S&P Overbought', type: 'warning', region: 'Global', desc: 'The S&P 500 is up over 1.2%, putting it in a technically overbought position.', action: 'Avoid chasing the market higher. Time to tighten stop-losses.', check: d => d.benchmarks?.['S&P 500']?.changePercent > 1.2 },
    { name: 'S&P Oversold', type: 'opportunity', region: 'Global', desc: 'The S&P 500 has sold off over 1.2%, putting it in an oversold condition.', action: 'Contrarian traders may look for signs of a bottom to initiate short-term longs.', check: d => d.benchmarks?.['S&P 500']?.changePercent < -1.2 },
    { name: 'Global Growth Confidence', type: 'opportunity', region: 'Global', desc: 'Small-cap stocks (Russell 2000) and crude oil are both rising.', action: 'Bullish for cyclical sectors like industrials, materials, and energy.', check: d => d.benchmarks?.['Russell 2000']?.changePercent > 0.8 && d.commodities?.['Crude Oil']?.changePercent > 1.0 },

    // ===================================
    // Sector & Style Signals
    // ===================================
    { name: 'Tech Dominance', type: 'info', region: 'Global', desc: `The Nasdaq is strongly outperforming the S&P 500.`, action: 'Favors long positions in technology stocks (QQQ).', check: d => d.benchmarks?.Nasdaq?.changePercent > d.benchmarks?.['S&P 500']?.changePercent + 0.5 },
    { name: 'Tech Weakness', type: 'warning', region: 'Global', desc: 'The Nasdaq is significantly underperforming the S&P 500.', action: 'Suggests caution on tech stocks. Reduce exposure to QQQ or look for shorts.', check: d => d.benchmarks?.['S&P 500']?.changePercent > d.benchmarks?.Nasdaq?.changePercent + 0.5 },
    { name: 'Value Over Growth', type: 'info', region: 'Global', desc: 'The S&P 500 is outperforming the Nasdaq.', action: 'Traders may look for opportunities in value-oriented ETFs.', check: d => d.benchmarks?.['S&P 500']?.changePercent > 0.2 && d.benchmarks?.Nasdaq?.changePercent < d.benchmarks?.['S&P 500']?.changePercent },
    { name: 'Small Cap Euphoria', type: 'warning', region: 'Global', desc: 'Small cap stocks (Russell 2000) are up more than 2.0%.', action: 'A signal to be cautious about market froth. High risk-taking could lead to a sharp reversal.', check: d => d.benchmarks?.['Russell 2000']?.changePercent > 2.0 },
    { name: 'Small Cap Panic', type: 'critical', region: 'Global', desc: 'Small cap stocks (Russell 2000) are down more than 2.0%.', action: 'Bearish for the broader economy. A strong signal to adopt a defensive posture.', check: d => d.benchmarks?.['Russell 2000']?.changePercent < -2.0 },
    { name: 'Small Caps Lead Higher', type: 'opportunity', region: 'Global', desc: 'Small caps are outperforming large caps (S&P 500).', action: 'Confirms bullish sentiment. Traders may favor small-cap ETFs (IWM).', check: d => d.benchmarks?.['Russell 2000']?.changePercent > d.benchmarks?.['S&P 500']?.changePercent + 0.4 },
    { name: 'Small Caps Lag', type: 'warning', region: 'Global', desc: 'Small caps are underperforming large caps (S&P 500).', action: 'A leading indicator of potential market weakness. Caution is advised.', check: d => d.benchmarks?.['S&P 500']?.changePercent > d.benchmarks?.['Russell 2000']?.changePercent + 0.4 },
    { name: 'Indian Banking Strength', type: 'opportunity', region: 'India', desc: 'The Bank Nifty is strongly outperforming the Nifty 50.', action: 'Bullish for the Indian market. Consider long positions in banking stocks.', check: d => d.specialized?.['Bank Nifty']?.changePercent > d.benchmarks?.['Nifty 50']?.changePercent + 0.6 },
    { name: 'Indian Banking Drag', type: 'warning', region: 'India', desc: 'The Bank Nifty is underperforming the Nifty 50.', action: 'A bearish sign for the Indian market. Avoid banking stocks or consider hedging.', check: d => d.benchmarks?.['Nifty 50']?.changePercent > d.specialized?.['Bank Nifty']?.changePercent + 0.6 },
    { name: 'Indian Financial Stress', type: 'critical', region: 'India', desc: 'The Bank Nifty is down more than 1.5%.', action: 'A strong signal to be cautious or bearish on the entire Indian market.', check: d => d.specialized?.['Bank Nifty']?.changePercent < -1.5 },
    ...
    { name: 'Tech vs Small Cap Rotation', type: 'info', region: 'Global', desc: 'Nasdaq and Russell 2000 are moving in opposite directions.', action: 'Traders can ride the rotation by favoring the outperforming group.', check: d => d.benchmarks?.Nasdaq?.changePercent * d.benchmarks?.['Russell 2000']?.changePercent < 0 },

    // ===================================
    // Commodity Signals
    // ===================================
    { name: 'Major Oil Price Move', type: 'critical', region: 'Global', desc: d => `Crude oil has moved by ${d.commodities?.['Crude Oil']?.changePercent?.toFixed(1)}%, impacting inflation and transportation stocks.`, action: 'High oil prices hurt airlines and consumer stocks but benefit energy producers.', check: d => Math.abs(d.commodities?.['Crude Oil']?.changePercent) > 3.0 },
    { name: 'Gold as Safe Haven', type: 'info', region: 'Global', desc: 'Gold is rising while the S&P 500 is falling, confirming its safe-haven status.', action: 'Traders may increase allocation to gold (GLD) as a hedge.', check: d => d.commodities?.Gold?.changePercent > 0.4 && d.benchmarks?.['S&P 500']?.changePercent < -0.4 },
    { name: 'Gold Losing Luster', type: 'warning', region: 'Global', desc: 'Gold is falling even as the S&P 500 sells off.', action: 'A sign of a liquidity crunch. Cash is king in this environment.', check: d => d.commodities?.Gold?.changePercent < -0.5 && d.benchmarks?.['S&P 500']?.changePercent < -0.5 },
    { name: 'Strong Gold Rally', type: 'opportunity', region: 'Global', desc: 'Gold is up more than 1.2%, a strong move.', action: 'Bullish for gold and gold mining stocks. Consider buying GLD.', check: d => d.commodities?.Gold?.changePercent > 1.2 },
    { name: 'Oil Plunge (Demand Fear)', type: 'warning', region: 'Global', desc: 'Crude oil has dropped more than 2.5%, signaling falling global demand.', action: 'Bearish for the global economy and energy stocks. Potentially bullish for transportation.', check: d => d.commodities?.['Crude Oil']?.changePercent < -2.5 },
    { name: 'Inflation Hedge Signal', type: 'warning', region: 'Global', desc: 'Gold is rising while bonds are selling off (yields rising).', action: 'A classic stagflation signal. Gold may outperform, while bonds and growth stocks may underperform.', check: d => d.commodities?.Gold?.changePercent > 0.6 && d.bonds?.['US 10Y Yield']?.changePercent > 1.5 },
    { name: 'Industrial Slowdown?', type: 'warning', region: 'Global', desc: 'Crude oil is falling while stocks are also falling.', action: 'A bearish signal for cyclical stocks. Defensive sectors may outperform.', check: d => d.commodities?.['Crude Oil']?.changePercent < -1.0 && d.benchmarks?.['S&P 500']?.changePercent < -0.8 },
    { name: 'Gold vs Dollar Inverse Correlation', type: 'info', region: 'Global', desc: 'Gold and the US Dollar are moving in opposite directions, as expected.', action: 'This is a normal market state. A falling dollar is typically a tailwind for gold prices.', check: d => d.commodities?.Gold?.changePercent * d.currencies?.DXY?.changePercent < 0 },
    { name: 'Gold Breaking Correlation', type: 'info', region: 'Global', desc: 'Gold and the US Dollar are rising together, an unusual "flight to safety" event.', action: 'A sign of significant global instability. Reduce risk across the board.', check: d => d.commodities?.Gold?.changePercent > 0.4 && d.currencies?.DXY?.changePercent > 0.4 },
    { name: 'Commodity Super Cycle?', type: 'opportunity', region: 'Global', desc: 'Gold and Oil are positive while the US Dollar is negative.', action: 'Suggests a potential trend in commodities. Consider long positions in commodity ETFs.', check: d => d.commodities?.Gold?.changePercent > 0.5 && d.commodities?.['Crude Oil']?.changePercent > 0.5 && d.currencies?.DXY?.changePercent < -0.3 },
    { name: 'Oil-Stock Divergence', type: 'warning', region: 'Global', desc: 'Crude oil is rising sharply, but stocks are falling.', action: 'Bearish for the broader market, especially consumer discretionary and industrial sectors.', check: d => d.commodities?.['Crude Oil']?.changePercent > 1.5 && d.benchmarks?.['S&P 500']?.changePercent < -0.3 },
    
    // ===================================
    // Currency Signals
    // ===================================
    { name: 'Major Dollar Move', type: 'critical', region: 'Global', desc: d => `The US Dollar Index has moved by over 0.8%, a large move affecting global assets.`, action: 'A strong dollar is typically bearish for commodities and emerging markets.', check: d => Math.abs(d.currencies?.DXY?.changePercent) > 0.8 },
    { name: 'Strong Dollar Headwind', type: 'critical', region: 'Global', desc: 'A rising dollar is putting pressure on both Equities and Commodities.', action: 'Bearish for most assets except the dollar itself. A time to be defensive.', check: d => d.currencies?.DXY?.changePercent > 0.4 && d.benchmarks?.['S&P 500']?.changePercent < 0 && d.commodities?.Gold?.changePercent < 0 },
    { name: 'Weak Dollar Tailwind', type: 'opportunity', region: 'Global', desc: 'A falling dollar is boosting both Equities and Commodities.', action: 'Bullish for most assets, especially commodities and international stocks.', check: d => d.currencies?.DXY?.changePercent < -0.4 && d.benchmarks?.['S&P 500']?.changePercent > 0 && d.commodities?.Gold?.changePercent > 0 },
    { name: 'Rupee Under Pressure', type: 'warning', region: 'India', desc: `The Indian Rupee has weakened by more than 0.3% against the USD.`, action: 'Negative for the Indian market. Can signal FII outflows.', check: d => d.currencies?.['USD/INR']?.changePercent > 0.3 },
    { name: 'Rupee Strengthening', type: 'opportunity', region: 'India', desc: 'The Indian Rupee is strengthening against the USD by more than 0.3%.', action: 'Positive for the Indian market, especially for companies with high import costs.', check: d => d.currencies?.['USD/INR']?.changePercent < -0.3 },
    ...
    { name: 'Indian IT Sector Boost', type: 'info', region: 'India', desc: 'A weakening Rupee is generally positive for Indian IT companies.', action: 'This macro condition can support a rally in Indian IT stocks.', check: d => d.currencies?.['USD/INR']?.changePercent > 0.2 },
    { name: 'Risk-On Currency Flow', type: 'opportunity', region: 'Global', desc: 'The US Dollar is weakening while high-risk small-cap stocks are rallying.', action: 'Confirms a "risk-on" environment. Bullish for equities.', check: d => d.currencies?.DXY?.changePercent < -0.4 && d.benchmarks?.['Russell 2000']?.changePercent > 0.8 },
    ...
    { name: 'Indian Importer Alert', type: 'warning', region: 'India', desc: 'A weakening Rupee can increase costs for Indian companies that rely on imports.', action: 'Potentially bearish for stocks in sectors with high import costs.', check: d => d.currencies?.['USD/INR']?.changePercent > 0.2 && d.commodities?.['Crude Oil']?.changePercent > 0 },
    
    // ===================================
    // Bond & Credit Market Signals
    // ===================================
    { name: 'Major Bond Sell-Off', type: 'critical', region: 'Global', desc: d => `The 10-Year Treasury Yield has jumped by more than 4%.`, action: 'Bearish for long duration assets like growth and tech stocks.', check: d => d.bonds?.['US 10Y Yield']?.changePercent > 4.0 },
    { name: 'Major Bond Rally', type: 'opportunity', region: 'Global', desc: 'The 10-Year Treasury Yield is falling by more than 4%.', action: 'Bullish for bonds and growth stocks. Can signal impending economic weakness.', check: d => d.bonds?.['US 10Y Yield']?.changePercent < -4.0 },
    { name: 'Yields Up, Stocks Up (Reflation)', type: 'info', region: 'Global', desc: 'Both bond yields and stocks are rising, a sign of a pro-growth environment.', action: 'Bullish for cyclical and value stocks. Suggests the economy can handle higher rates.', check: d => d.bonds?.['US 10Y Yield']?.changePercent > 1.0 && d.benchmarks?.['S&P 500']?.changePercent > 0.5 },
    { name: 'Yields Up, Stocks Down (Stagflation Fear)', type: 'warning', region: 'Global', desc: 'Bond yields are rising but stocks are falling.', action: 'A difficult environment. Bearish for most stocks, especially growth.', check: d => d.bonds?.['US 10Y Yield']?.changePercent > 1.0 && d.benchmarks?.['S&P 500']?.changePercent < -0.3 },
    { name: 'Yields Down, Stocks Up (Goldilocks)', type: 'opportunity', region: 'Global', desc: 'Bond yields are falling and stocks are rising, a very bullish scenario.', action: 'The best environment for stocks, especially growth and technology.', check: d => d.bonds?.['US 10Y Yield']?.changePercent < -1.0 && d.benchmarks?.['S&P 500']?.changePercent > 0.5 },
    { name: 'Yields Down, Stocks Down (Recession Fear)', type: 'critical', region: 'Global', desc: 'Both bond yields and stocks are falling, a strong signal of recession fears.', action: 'Highly defensive signal. Favors holding government bonds and cash.', check: d => d.bonds?.['US 10Y Yield']?.changePercent < -1.0 && d.benchmarks?.['S&P 500']?.changePercent < -0.5 },
    { name: 'Credit Market Stress', type: 'critical', region: 'Global', desc: 'High-yield ("junk") bonds are selling off sharply.', action: 'A leading indicator of economic trouble. Strongly bearish for the stock market.', check: d => d.bonds?.['High-Yield Bonds']?.changePercent < -0.8 },
    { name: 'Credit Market Rally', type: 'opportunity', region: 'Global', desc: 'High-yield bonds are rallying, signaling investor confidence.', action: 'A leading indicator of economic strength. Bullish for the stock market.', check: d => d.bonds?.['High-Yield Bonds']?.changePercent > 0.6 },
    { name: 'Growth Stock Warning', type: 'warning', region: 'Global', desc: 'Treasury yields are rising sharply, negatively impacting tech stocks.', action: 'A signal to be cautious with tech stocks (Nasdaq). May favor a rotation to value.', check: d => d.bonds?.['US 10Y Yield']?.changePercent > 2.0 && d.benchmarks?.Nasdaq?.changePercent < 0 },
    { name: 'Defensive Rotation', type: 'info', region: 'Global', desc: 'Bonds are rallying while high-risk small caps are selling off.', action: 'Confirms a risk-off sentiment. Reduce equity exposure and increase bond allocation.', check: d => d.bonds?.['US 10Y Yield']?.changePercent < -0.8 && d.benchmarks?.['Russell 2000']?.changePercent < -0.8 },
    { name: 'High Yield Divergence (Bearish)', type: 'warning', region: 'Global', desc: 'The S&P 500 is rising but high-yield bonds are falling.', action: 'A strong warning sign that the equity rally may be deceptive and could fail.', check: d => d.benchmarks?.['S&P 500']?.changePercent > 0.4 && d.bonds?.['High-Yield Bonds']?.changePercent < 0 },
    { name: 'High Yield Confirmation (Bullish)', type: 'opportunity', region: 'Global', desc: 'Both the S&P 500 and high-yield bonds are rising together.', action: 'A strong confirmation of a healthy rally. Supports holding or adding to long equity positions.', check: d => d.benchmarks?.['S&P 500']?.changePercent > 0.5 && d.bonds?.['High-Yield Bonds']?.changePercent > 0.3 },

    // ===================================
    // India-Specific Signals
    // ===================================
    { name: 'India Outperformance', type: 'opportunity', region: 'Intermarket', desc: d => `The Nifty 50 is outperforming the S&P 500 by ${(d.benchmarks?.['Nifty 50']?.changePercent - d.benchmarks?.['S&P 500']?.changePercent).toFixed(1)}%.`, action: 'Suggests favoring Indian equities over US equities.', check: d => d.benchmarks?.['Nifty 50']?.changePercent > d.benchmarks?.['S&P 500']?.changePercent + 0.4 },
    { name: 'India Underperformance', type: 'warning', region: 'Intermarket', desc: 'The Nifty 50 is significantly underperforming global markets.', action: 'Suggests reducing exposure to Indian equities.', check: d => d.benchmarks?.['S&P 500']?.changePercent > d.benchmarks?.['Nifty 50']?.changePercent + 0.5 },
    { name: 'Indian Market Sell-Off', type: 'critical', region: 'India', desc: 'The Nifty 50 is down more than 1.0%.', action: 'A clear bearish signal for the day. Traders may liquidate long positions.', check: d => d.benchmarks?.['Nifty 50']?.changePercent < -1.0 },
    { name: 'Indian Market Breakout', type: 'opportunity', region: 'India', desc: 'The Nifty 50 is up more than 1.0%.', action: 'A strong bullish signal. Traders may initiate long positions.', check: d => d.benchmarks?.['Nifty 50']?.changePercent > 1.0 },
    { name: 'Indian Decoupling (Bullish)', type: 'opportunity', region: 'Intermarket', desc: 'The Indian market is rising even as major global markets are falling.', action: 'Very bullish for India. Suggests Indian equities are a good place to be relative to the world.', check: d => d.benchmarks?.['Nifty 50']?.changePercent > 0.4 && d.benchmarks?.['S&P 500']?.changePercent < -0.1 },
    { name: 'Indian Decoupling (Bearish)', type: 'warning', region: 'Intermarket', desc: 'The Indian market is falling even as global markets are rising.', action: 'A signal to be cautious specifically on Indian assets.', check: d => d.benchmarks?.['Nifty 50']?.changePercent < -0.4 && d.benchmarks?.['S&P 500']?.changePercent > 0.1 },
    { name: 'Broad Indian Rally', type: 'opportunity', region: 'India', desc: 'Both the Nifty 50 and Bank Nifty are rising strongly.', action: 'Confirms a healthy rally in the Indian market. Supports holding long positions.', check: d => d.benchmarks?.['Nifty 50']?.changePercent > 0.8 && d.specialized?.['Bank Nifty']?.changePercent > 1.0 },
    ...
    { name: 'Nifty Range-Bound Day', type: 'info', region: 'India', desc: 'The Nifty 50 is trading in a narrow range (less than +/- 0.3%).', action: 'A signal for range-trading strategies like selling straddles/strangles.', check: d => Math.abs(d.benchmarks?.['Nifty 50']?.changePercent) < 0.3 },
    { name: 'Nifty-Sensex Divergence', type: 'warning', region: 'India', desc: 'The Nifty 50 and Sensex are moving in opposite directions.', action: 'A sign of a confusing market. Wait for a clearer trend to emerge.', check: d => (d.benchmarks?.['Nifty 50']?.changePercent * d.benchmarks?.Sensex?.changePercent) < 0 },
    { name: 'India VIX - High Fear Zone', type: 'critical', region: 'India', desc: 'The India VIX is above 20, indicating high fear and uncertainty.', action: 'A time for caution. Reduce position sizes. Good environment for option buyers.', check: d => d.volatility?.['India VIX']?.price > 20 },
    { name: 'Bearish Complacency (India)', type: 'warning', region: 'India', desc: 'The Nifty 50 is falling, but the India VIX is also falling.', action: 'A signal that the downtrend may continue. Opportunity to add to short positions.', check: d => d.benchmarks?.['Nifty 50']?.changePercent < -0.3 && d.volatility?.['India VIX']?.changePercent < 0 },
    { name: 'Bullish Confidence (India)', type: 'opportunity', region: 'India', desc: 'The Nifty 50 is rising, and the India VIX is falling.', action: 'Confirms a bullish trend. Supports holding or adding to long positions.', check: d => d.benchmarks?.['Nifty 50']?.changePercent > 0.3 && d.volatility?.['India VIX']?.changePercent < -1.0 },
    { name: 'FII Outflow Warning', type: 'critical', region: 'India', desc: 'The Rupee is weakening and the Nifty 50 is falling.', action: 'A strong bearish signal. FII selling can lead to sustained downturns.', check: d => d.currencies?.['USD/INR']?.changePercent > 0.2 && d.benchmarks?.['Nifty 50']?.changePercent < -0.4 },
    { name: 'FII Inflow Signal', type: 'opportunity', region: 'India', desc: 'The Rupee is strengthening and the Nifty 50 is rising.', action: 'A strong bullish signal. FII buying can fuel powerful rallies.', check: d => d.currencies?.['USD/INR']?.changePercent < -0.2 && d.benchmarks?.['Nifty 50']?.changePercent > 0.4 }
];
  ...
  function toggleView(btn, view) {
      ...
      if (view === 'library') {
          ...
          // When switching back to a data view, refresh the data
  ...
  function renderSignalLibrary() {
      ...
      libraryContainer.innerHTML = signalsConfig.map((signal, index) => {
          ...
          return `
              <div class="library-card ${isActive ? 'active-card' : ''}">
  ...
  // --- PASTE ALL OTHER JS FUNCTIONS FROM THE PREVIOUS TURN HERE ---
  // (renderCharts, fetchApi, fetchYahooFinanceData, etc., ending with the window.addEventListener('load',...))
  // This is to avoid making the response excessively long, as these functions have not changed.
  // Just ensure this new/modified code replaces the old code at the top of your app.js.
