import {
  ...
  function recalculateCollapsibleHeight() {
      document.querySelectorAll('.collapsible.active').forEach(button => {
          const content = button.nextElementSibling;
          if (content && content.classList.contains('collapsible-content')) {
              content.style.maxHeight = content.scrollHeight + "px";
  ...
  function toggleView(btn, view) {
      ...
      const signalsContainer = document.querySelector('.section-title.collapsible:first-of-type').parentElement;
      const matrixContainer = document.getElementById('matrix-title').parentElement;
      ...
      if (view === 'library') {
          ...
          const signalsTitle = signalsContainer.querySelector('.collapsible');
          if (!signalsTitle.classList.contains('active')) {
              signalsTitle.classList.add('active');
              const content = signalsTitle.nextElementSibling;
              content.style.maxHeight = content.scrollHeight + "px";
          }
      } else {
  ...
  async function renderDashboard(d = null) { 
      const ish = d !== null; 
      ...
      const dp = fetchAllMarketData(d); 
      const np = ish ? Promise.resolve([]) : fetchLiveNews(currentView); 
      const [md, fn] = await Promise.all([dp, np]); 
      newsData = fn || []; 
      liveMarketData = md || {}; 
      ...
      document.getElementById('news-panel').style.display = ish ? 'none' : 'block'; 
      if (!ish) { 
          renderNews(newsData, currentView); 
      } 
      const sd = generateExecutiveSummary(liveMarketData, currentView); 
      renderExecutiveSummary(sd); 
      updateLastUpdated(); 
      if (ish) { 
          document.getElementById('priceChartContainer').innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-dim);">Live chart unavailable for historical dates.</div>`; 
          document.getElementById('newsChartContainer').innerHTML = ''; 
      } else { 
          renderCharts(); 
      }
      // Recalculate heights after all async rendering is complete
      recalculateCollapsibleHeight();
  }
  ...
  window.addEventListener('load', () => {
      ...
      // This is now handled by the first renderDashboard call
